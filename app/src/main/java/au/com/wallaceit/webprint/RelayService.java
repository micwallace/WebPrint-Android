        ...
        public Response serve(IHTTPSession session) {
            ...
            if (Method.GET.equals(method)) {
                if (session.getUri().equals("/printwindow")){
                    responseBody = "<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"/><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"/><style>h1, h2 { color:#0078ae; font-family:helvetica; font-size:110%; }</style><script>window.addEventListener('message',sendData); function sendData(event){ var xmlhttp=new XMLHttpRequest(); xmlhttp.open('POST','http://127.0.0.1:8080/',false); xmlhttp.send(event.data); } window.opener.postMessage('init', '*'); </script></head>";
